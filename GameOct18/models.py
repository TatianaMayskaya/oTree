...
class Constants(BaseConstants):
    ...
    choice1_RiskyUrns_ru = 'Лотерея'
    choice1_RiskyUrns_en = 'Urn gamble'
...
class Subsession(BaseSubsession):
    def creating_session(self):
        ...
        keep_give_amounts = []
        for offer in Constants.offer_choices:
            keep_give_amounts.append((offer, Constants.endowment - offer))
        ...
        for give, keep in keep_give_amounts:
            if language == 1:
                ...
                offer_options_in_words.append(
                    (give, 'Игрок 2 получает {} жетонов (у Вас останется {} жетонов)'.format(give, keep)))
        ...
        if language == 1:
            ...
            quiz_question.append(
                'In game 1, will player 2 see the offer before or after she makes her move in the game?')
            ...
            question_RiskyProject1 = 'Please choose how many tokens you want to invest in the risky project. ' \
                                     ...
                                     str(Constants.endowment_RiskyProject1) + ', including 0 or ' + \
                                     str(Constants.endowment_RiskyProject1)
            question_RiskyProject2 = 'Please choose how many tokens you want to invest in the risky project. ' \
                                     'Note that you can pick any number between 0 and ' + \
                                     str(Constants.endowment_RiskyProject2) + ', including 0 or ' + \
                                     str(Constants.endowment_RiskyProject2)
            label_RiskyUrns = 'Choose one option'
            choice1_RiskyUrns = Constants.choice1_RiskyUrns_en
            ...
            SurveyPersonal_question.append(
                preambul + '\"If you had a business, you would find it very difficult to fire '
            ...
            SurveyPersonal_question.append(preambul + '\"Keeping your options open is more important than having '
                                                      'a to-do list.\"')
            SurveyPersonal_question.append(
                preambul + '\"You think that everyone\'s views should be respected regardless '
            ...
            question_RiskyProject1 = 'Пожалуйста, выберите, сколько жетонов Вы проинвестируете в рискованный проект. ' \
                                     ...
                                     str(Constants.endowment_RiskyProject1) + ', включая 0 и ' + \
                                     str(Constants.endowment_RiskyProject1)
            question_RiskyProject2 = 'Пожалуйста, выберите, сколько жетонов Вы проинвестируете в рискованный проект. ' \
                                     'Вы можете выбрать любое число от 0 до ' + \
                                     str(Constants.endowment_RiskyProject2) + ', включая 0 и ' + \
                                     str(Constants.endowment_RiskyProject2)
            label_RiskyUrns = 'Выберите одну опцию'
            choice1_RiskyUrns = Constants.choice1_RiskyUrns_ru
            ...
            SurveyPersonal_question.append(
                preambul + '\"Выиграть спор для меня менее важно, чем не расстроить других\".')
...
def make_primary_field(num):
    ...
    return models.IntegerField(
        choices=Subsession.offer_options_in_words,
...
def make_survey1_question_str(lnum, num):
    if lnum == 1:
        return models.LongStringField(
            label=Subsession.survey1_question_1[num-1]
        )
    elif lnum == 3:
        return models.LongStringField(
            label=Subsession.survey1_question_3[num - 1]
        )
    elif lnum == 4:
        return models.LongStringField(
            blank=True,
            label=Subsession.survey1_question_4[num - 1]
        )
    else:
        return []
...
def make_survey1_question(num):
    return models.IntegerField(
        choices=[1, 2, 3, 4, 5],
        widget=widgets.RadioSelectHorizontal,
        label=Subsession.survey1_question_2[num-1]
    )


def make_survey2_urn(num):
    return models.IntegerField(
        choices=[[1, Subsession.choice1_RiskyUrns], [2, str(Subsession.choice2_RiskyUrns).format(num)]],
        widget=widgets.RadioSelectHorizontal,
        verbose_name=Subsession.label_RiskyUrns
    )
...
def make_survey_personal(num):
    return models.IntegerField(
        ...
        widget=widgets.RadioSelectHorizontal,
        label=Subsession.SurveyPersonal_question[num-1])
...
class Player(BasePlayer):
    ...
    quiz_questions_1 = models.StringField(
        choices=Subsession.quiz_answer_1,
        label=Subsession.quiz_question[0],
        widget=widgets.RadioSelectHorizontal
    )
    quiz_questions_2 = models.StringField(
        choices=Subsession.quiz_answer_2,
        label=Subsession.quiz_question[1],
        widget=widgets.RadioSelectHorizontal
    )
    quiz_questions_3 = models.IntegerField(
        label=Subsession.quiz_question[2],
    )
    quiz_questions_4 = models.IntegerField(
        label=Subsession.quiz_question[3],
    )
    quiz_questions_5 = models.IntegerField(
        label=Subsession.quiz_question[4],
    )
    quiz_questions_6 = models.StringField(
        choices=Subsession.quiz_answer_6,
        label=Subsession.quiz_question[5],
        widget=widgets.RadioSelectHorizontal
    )
    quiz_questions_7 = models.StringField(
        choices=Subsession.quiz_answer_7,
        label=Subsession.quiz_question[6],
        widget=widgets.RadioSelectHorizontal
    )
    quiz_questions_8 = models.IntegerField(
        label=Subsession.quiz_question[7],
    )
    quiz_questions_9 = models.StringField(
        choices=Subsession.quiz_answer_9,
        label=Subsession.quiz_question[8],
        widget=widgets.RadioSelectHorizontal
    )
    quiz_questions_10 = models.StringField(
        choices=Subsession.quiz_answer_10,
        label=Subsession.quiz_question[9],
        widget=widgets.RadioSelectHorizontal
    )

    def check_correct(self, p):
        answer = getattr(self, 'quiz_questions_{}'.format(p))
        return answer == Subsession.quiz_correct_answers[p-1]

    def number_correct(self):
        n = 0
        for p in Subsession.quiz_questions_range:
    ...
    def set_final_payoff(self):
        ...
        if self.payoff_game == 1:
            self.payoff = Subsession.show_up + self.payoff_G1 * Subsession.rate
        elif self.payoff_game == 2:
            self.payoff = Subsession.show_up + self.payoff_G2 * Subsession.rate
        else:
            self.payoff = Subsession.show_up + self.payoff_G3 * Subsession.rate
        self.payoff_text = add_currency(Subsession.currency_used, int(self.payoff))
    ...
    survey2_question_RiskyProject1 = models.IntegerField(
        ...
        label=Subsession.question_RiskyProject1)
    ...
    survey2_question_RiskyProject2 = models.IntegerField(
        ...
        label=Subsession.question_RiskyProject2)
    ...
    def get_payoff_survey2(self):
        ...
        if num == 1:
            ...
            self.survey2_RiskyUrns1_sure = str(Subsession.choice2_RiskyUrns).format(sure)
            self.survey2_RiskyUrns1_choice_num = getattr(self, 'survey2_RiskyUrns1_{}'.format(sure))
            if self.survey2_RiskyUrns1_choice_num == 1:
                self.survey2_RiskyUrns1_choice = Subsession.choice1_RiskyUrns
            ...
            self.survey2_RiskyUrns2_sure = str(Subsession.choice2_RiskyUrns).format(sure)
            self.survey2_RiskyUrns2_choice_num = getattr(self, 'survey2_RiskyUrns2_{}'.format(sure))
            if self.survey2_RiskyUrns2_choice_num == 1:
                self.survey2_RiskyUrns2_choice = Subsession.choice1_RiskyUrns
    ...
    survey_personal_questions_1 = models.StringField(
        choices=[Subsession.SurveyPersonal_question[0][1], Subsession.SurveyPersonal_question[0][2],
                 Subsession.SurveyPersonal_question[0][3]],
        widget=widgets.RadioSelectHorizontal,
        label=Subsession.SurveyPersonal_question[0][0])
    survey_personal_questions_2 = models.IntegerField(
        min=0, max=150,
        label=Subsession.SurveyPersonal_question[1])
    survey_personal_questions_3 = models.StringField(
        label=Subsession.SurveyPersonal_question[2])
    survey_personal_questions_4 = models.IntegerField(
        min=0, max=50,
        label=Subsession.SurveyPersonal_question[3])
