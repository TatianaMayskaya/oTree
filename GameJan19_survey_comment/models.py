...
doc = """
Survey. Comments (not obligatory questions)
"""


class Constants(BaseConstants):
    name_in_url = 'GameJan19_survey_comment'
    players_per_group = None

    # this is done only to count the number of questions
    # (assuming Russian and English versions have the same number)
    with open('GameJan19_intro/survey_comment_en.csv') as questions_file:
...
class Player(BasePlayer):
    ...
    def current_question(self):
        return self.session.vars['survey_comment_list'][self.round_number - 1]
