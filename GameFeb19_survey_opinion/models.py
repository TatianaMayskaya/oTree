    ...
    def creating_session(self):
        ...
        if self.round_number == 1:
            if self.session.config['language'] == 'en':
                with open('GameFeb19_survey_opinion/survey_opinion_en.csv') as questions_file:
                    self.session.vars['survey_opinion_list'] = list(csv.DictReader(questions_file))
            else:
                with open('GameFeb19_survey_opinion/survey_opinion_ru.csv', encoding='utf-8') as questions_file:
                    self.session.vars['survey_opinion_list'] = list(csv.DictReader(questions_file))
